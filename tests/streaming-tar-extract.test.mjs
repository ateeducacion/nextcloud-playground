...
describe("StreamingTarParser", () => {
  ...
  it("11. surfaces an empty directory written by createUstarTar", () => {
    // End-to-end regression: a genuinely-empty directory must survive the
...
describe("extractTarStreamToPhp", () => {
  it("creates an empty directory on the filesystem via mkdirTree", async () => {
    ...
    assert.ok(
      mkdirs.includes("/www/nextcloud/emptydir"),
      `expected /www/nextcloud/emptydir to be created; got ${JSON.stringify(mkdirs)}`,
    );
    assert.ok(writes.includes("/www/nextcloud/mod/pkg/version.php"));
    assert.equal(stats.dirCount, 1);
    assert.equal(stats.fileCount, 1);
  });
});

...
describe("formatBlueprintText", () => {
  it("pretty-prints valid JSON with 2-space indentation", () => {
    assert.strictEqual(
      formatBlueprintText('{"steps":[{"step":"enableApp"}]}'),
      '{\n  "steps": [\n    {\n      "step": "enableApp"\n    }\n  ]\n}',
    );
  });
...
describe("createBlueprintValidationResult", () => {
  // A stand-in for this repo's normalizeBlueprint(): permissive for any
  // plain-object input, throws for anything else — matching the real
  // src/shared/blueprint.js contract used by blueprint-editor.js.
  ...
  it("returns a valid result with the normalized blueprint", () => {
    ...
    assert.deepStrictEqual(result.blueprint, { steps: [], normalized: true });
  });
});
...
describe("buildBlueprintRunUrl", () => {
  it("sets the blueprint param and removes blueprint-url and blueprint-data", () => {
    const url = buildBlueprintRunUrl(
      "https://example.com/app/?blueprint-url=https://old.example/bp.json&blueprint-data=old&foo=bar",
      "ENCODED",
    );
  ...
  it("preserves the origin and path", () => {
    const url = buildBlueprintRunUrl(
      "https://example.com/nextcloud-playground/index.html?x=1",
      "abc",
    );
    assert.ok(
      url.startsWith("https://example.com/nextcloud-playground/index.html?"),
    );
  });
});

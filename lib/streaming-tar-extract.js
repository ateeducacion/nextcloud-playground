...
// tar.zst core bundle (see docs/streaming-tar-zst-core-bundle.md). Replaces the
// old "write the whole ZIP into MEMFS then ZipArchive::extractTo()" boot path.
//
...
// uncompressed tar (~250 MB-class for a large app core) is NEVER materialized:
// at any moment we hold only a partial 512-byte header, the current entry's
// bytes (bounded by the largest single file), and one decoded chunk.
//
// Path safety mirrors the ZIP boot path (lib/nextcloud-loader.js): "\\"→"/",
...
function readCString(block, offset, length) {
  ...
  return TEXT_DECODER.decode(block.subarray(offset, end));
}
...
export class StreamingTarParser {
  ...
  _finishEntry() {
    ...
    if (entry.isLongLink) {
      ...
      this.pendingLongName = TEXT_DECODER.decode(data).replace(/\0.*$/, "");
      return;
    }
 ...
 * via the raw Emscripten module (mkdirTree + writeFile) without ever holding the

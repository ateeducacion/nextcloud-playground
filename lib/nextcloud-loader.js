...
import { createDecodedTarStream, StreamingTarParser } from "./streaming-tar-extract.js";
...
/**
 * Wrap a boot-path fetch so a network-level failure names what it was loading.
 *
...
/**
 * Resolve the absolute URL of every part of a chunked (multi-part) bundle.
 */
...
/**
 * Download the core bundle with Cache API caching and SHA-256 verification.
 */
...
/**
 * Sanitize an archive entry path to prevent archive-slip (path traversal). Normalizes
...
export function extractZipEntries(zipBytes) {
  ...
  for (const [rawPath, data] of Object.entries(raw)) {
    ...
    if (!path) continue;
    // Reject archive-slip: skip entries that escape the target root via ".." or
...
/**
 * Sniff an archive's format from its magic bytes. Nextcloud's own app/appstore
 * packaging (`make dist` / `make appstore`) always produces a gzip-compressed
 * tar, not a ZIP, so `installApp`/`unzip` need to tell the two apart to pick
 * the right extractor.
 */
...
/**
 * Extract entries from a gzip-compressed tar (.tar.gz), normalize paths, and
 * strip a common leading folder — mirrors extractZipEntries() so both formats
 * produce the same { path, data } shape for writeEntriesToPhp(). Unlike the
 * bounded-memory core-bundle path (extractTarStreamToPhp), this collects
 * entries in memory: Nextcloud app packages are a few MB, not the ~70MB/
 * 3000-file editor bundle that motivated the streaming writer.
 */
export async function extractTarGzEntries(bytes) {
  const stream = await createDecodedTarStream(bytes, "gzip");
  const reader = stream.getReader();
  const raw = [];
  const parser = new StreamingTarParser({
    onEntry: (entry) => {
      if (entry.type === "file") raw.push(entry);
    },
  });
  for (;;) {
    const { done, value } = await reader.read();
  ...
  const paths = raw.map((entry) => entry.path);
  let prefix = "";
  if (paths.length > 0) {
    const first = paths[0];
    const slashIndex = first.indexOf("/");
  ...
  for (const entry of raw) {
    ...
    if (!path) continue;
    entries.push({ path, data: entry.data });
  }
  return entries;
}

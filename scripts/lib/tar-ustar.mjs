...
// streaming parser (lib/streaming-tar-extract.js) and PHP's `PharData` reader both
// silently IGNORE PAX `path` records and would write long files under their
// truncated 100-byte name, colliding and dropping files. The USTAR prefix split
// and GNU longlink are read correctly by that parser, bsdtar and GNU tar, so this
// format keeps full file-count parity with the ZIP baseline.
//
...
// normalizeEntries) use typeflag '5' so directories that carry meaning on their
// own survive even when they hold no files.
//
// Reused semantics: entry-name sanitization mirrors sanitizeArchivePath() in
// lib/nextcloud-loader.js (reject "..", strip leading "/", drop "." segments) so
// the tar cannot carry a path-traversal entry — parity with the ZIP boot path.
 ...
 * recreates. Without this a genuinely-empty directory in the source tree would
 * vanish from the extracted filesystem, since the streaming extractor only
 * recreates directories from the parent path of each file it writes. Directories
 * implied by a file (every ancestor path of a kept file) are NOT emitted as
 * redundant members.
 *

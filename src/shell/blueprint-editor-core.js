 ...
 *
 * `deps.normalizeBlueprint` takes the parsed JSON value and either returns a
 * normalized blueprint object or throws (e.g. when the parsed value isn't a
 * plain object) — this repo's `normalizeBlueprint()` from
 * `src/shared/blueprint.js` otherwise never rejects a blueprint's *shape*, it
 * just fills in defaults, so there is no separate multi-error schema
 * validator to call here.
 *
 * @param {string} rawText
 ...
 * `blueprint=<encodedBlueprint>` and removes the `blueprint-url` and
 * `blueprint-data` (legacy alias) params, preserving everything else on the
 * current URL.
 *

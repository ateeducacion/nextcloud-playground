...
export function initBlueprintEditor(elements, options = {}) {
  ...
  // during boot — well after this editor is initialized. `normalizeBlueprint`
  // tolerates a missing config (all its field reads are optional-chained or
  // `||`-defaulted), so `config || {}` here is safe at any point in the
  // lifecycle and always reflects the latest loaded config.

 ...
 * bundle (~167MB / 15,496 files) that peak risks MEMFS OOM on constrained
 ...
 * code — fast regardless of file count and ~one-entry peak.
 *
 * The bundle wraps everything in a single top-level folder (e.g. `nextcloud/`);
 * we extract to a stage, descend into that lone wrapper when present (root-level
 * bundles have several top entries and are used as-is), then move it into place.
 *
 * Contract: prints exactly one sentinel on stdout:
...
echo (function () {
  ...
  try {
    ...
    $zip->close();
    if (!$ok) { $rrmdir($stage); return 'INSTALL_ERR extract'; }
    // Descend into a lone wrapping folder (e.g. "nextcloud/"); root-level
    // bundles keep several top entries and are used as-is.

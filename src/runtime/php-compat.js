/**
 * Compatibility layer that wraps @php-wasm/web's PHP instance
 * to match the API surface expected by the Nextcloud Playground
 * runtime (bootstrap.js, php-worker.js, addons.js, vfs.js).
 */

import { __private__dont__use } from "@php-wasm/universal";

const DEFAULT_WEB_ROOT = "/www/nextcloud";
...
/**
 * Wraps a @php-wasm/web PHP instance with the compatibility API
 * expected by the Nextcloud Playground runtime.
 */
export function wrapPhpInstance(
  ...
  return {
    ...
    async request(requestOrUrl) {
      ...
      // For non-PHP paths that don't exist as files, fall through to the
      // front controller (index.php) — this is the equivalent of Apache's
      // mod_rewrite "RewriteCond %{REQUEST_FILENAME} !-f" rule that
      // Nextcloud relies on for clean URLs like /Dashboard, /admin, etc.
      if (!isPhpScript(scriptPath)) {
        try {
          ...
          // File doesn't exist — fall through to front controller below
        }
      }

      // Route through the front controller for non-existent PHP scripts
      // or non-file paths (clean URL rewriting).
      const effectiveScriptPath =
        isPhpScript(scriptPath) && php.fileExists(scriptPath)
          ? scriptPath
          : `${resolvedWebRoot}/index.php`;
      ...
      const serverVars = {
        ...
        HTTP_USER_AGENT: "NextcloudPlayground/1.0 (WASM)",
      ...
      const scriptDir =
        effectiveScriptPath.substring(
          0,
          effectiveScriptPath.lastIndexOf("/"),
        ) || "/";
      try {
      ...
      // Remember cookies from Set-Cookie headers
